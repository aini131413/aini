window.api = {
  // 登录验证接口
  loginCheck: '/api/remember/login',
  // 获取短信验证码接口
  sms: '/api/remember/sms',
  // 注册接口
  regist: '/api/remember/regist',
  // 修改密码接口
  changePassword: '/api/remember/edit',
  // 忘记密码接口
  forgetPassword: '/api/remember/forget',
  // 地址列表接口
  addressList: '/api/address/list',
  // 指定地址接口
    address: '/api/address/detail',
  // 保存新增地址接口
  addAddress: '/api/address/add',
  // 保存修改地址接口
  editAddress: '/api/address/edit',
  // 获得订单列表接口
  orderList: '/api/order/list',
  // 获得指定订单接口
  order: '/api/order/detail',
  // 获得商家列表接口
  businessList: '/api/shop/list',
  // 获得指定商家接口
  business: '/api/shop/index',
  // 保存购物车接口
  addCart: '/api/cart/add',
  // 获取购物车数据接口
  cart: '/api/cart/list',
  // 添加订单接口
  addOrder: '/api/order/add',
  // 支付接口
  pay: '/api/remember/pay',
    // 用户详情接口
  userDetail: 'api/remember/detail'
};
